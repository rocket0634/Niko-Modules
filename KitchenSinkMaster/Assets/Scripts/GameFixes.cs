...
#region KMSelectable.UpdateChildren fix
    /*
     * Bug description:
     * Adding children to a KMSelectable by instantiating prefabs with KMSelectable components on them doesn't work.
     * When the game loads a mod, it adds the internal types to all of the prefabs in it for all the proxy types (KM...) it finds (here: KMSelectable -> ModSelectable).
     ...
     *  but the internal ModSelectable.OnUpdateChildren method would only update these settings if the selectable wouldn't have the ModSelectable component, but the component got added when the mod was loaded, thus the settings are never updated.
     *
